...
const WARDROBE = [
  ...
  { name:"Schal_rotgrau_Office-Freizeit",            folder:"Accessoires", cat:"Accessoire" },
  // HEMDEN – Kurzarm
  { name:"Hemd_kurzarm_blau_grau_kariiert_Lager-Freizeit",          folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_kurzarm_blau_weiss_gestreift_Office-Freizeit",       folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_kurzarm_blau_weiss_gsteift_Office-Lager-Freizeit",   folder:"Hemden", cat:"Oberteil" },
  ...
  { name:"Hemd_kurzarm_gelb_Orientalisch_Freizeit",                 folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_kurzarm_schwarz_CampDavid_Lager-Freizeit",           folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_kurzarm_grossgemustert_Freizeit",                    folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_kurzarm_weiss_getigert_Boss_Freizeit",               folder:"Hemden", cat:"Oberteil" },
  ...
  { name:"Hemd_kurzarm_blau_schwarz_gemustert_Office-Freizeit",     folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_kurzarm_petrohlschwarz_gemustert_Lager-Freizeit",    folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_kurzarm_rotblau_Wap_gel_Lager-Freizeit",            folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_kurzarm_schwarz_gemustert_Office",                   folder:"Hemden", cat:"Oberteil" },
  // HEMDEN – Langarm
  { name:"Hemd_langarm_Baumwolle_Blauschwarz_Office-Freizeit",      folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_langarm_belgisch_gemustert_Office-Freizeit",         folder:"Hemden", cat:"Oberteil" },
  ...
  { name:"Hemd_langarm_blau_weiss_Office-Freizeit",                 folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_langarm_Cord_ocker_Lager",                           folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_langarm_Denim_dunkelblau_Office-Freizeit",           folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_langarm_dunkelblau_Gore-Tex_Office-Freizeit",        folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_langarm_dunkelblau_Olymp_Office-Freizeit",           folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_langarm_farbige_Karos_Office-Freizeit",              folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_langarm_farbig_gemustert_Office-Freizeit",           folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_langarm_Flanell_gruengrau_Lager",                    folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_langarm_grau_gemustert_Office-Freizeit",             folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_langarm_gruenblau_Office-Freizeit",                  folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_langarm_hellblau_gemustert_Office-Freizeit",         folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_langarm_jeansbau_Lager",                             folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_langarm_Joop_grau_gemustert_Office-Freizeit",        folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_langarm_kariert_Office-Freizeit",                    folder:"Hemden", cat:"Oberteil" },
  ...
  { name:"Hemd_langarm_schwarz_grau_Office-Freizeit",               folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_Overshirt_grau_Lager-Freizeit",                      folder:"Hemden", cat:"Oberteil" },
  { name:"Hemd_Overshirt_grauAnthrazit",                            folder:"Hemden", cat:"Oberteil" },
  // JACKEN
  ...
  { name:"WoolOver_Strickpullover_Office-Lager-Freizeit",           folder:"Pullover", cat:"Oberteil" },
  // SCHUHE
  ...
  { name:"Schuhe_Sneaker_grau_Office-Freizeit",                     folder:"Schuhe", cat:"Schuhe" },
  ...
  { name:"Hose_Chino_blau_Office-Freizeit",                  folder:"Hosen", cat:"Hose" },
  ...
  { name:"Hose_Leinen_leicht_Freizeit",                      folder:"Hosen", cat:"Hose" },
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          system: `Du bist ein persönlicher Stilberater. Stelle ein stimmiges Outfit zusammen.
...
OPTIONAL (max je 1):
- Jacke [Jacke]
- Weste [Weste] – wird UNTER der Jacke getragen
...
Gib NUR die exakten Dateinamen aus der Liste zurück – keine Erfindungen.
Antworte NUR mit diesem JSON (keine Backticks):
{"greeting":"1 Satz","outfit_description":"2 Saetze","items":[{"name":"EXAKTER_DATEINAME","cat":"Kategorie"}]}`,
      ...
      let items = (parsed.items || [])
        .map(item => {
          const entry = byName[item.name];
          if (!entry) {
            console.warn("⚠️ Unbekannter Name von Claude:", JSON.stringify(item.name));
            // Fuzzy-Fallback: ersten Treffer mit gleichem Anfang suchen
            const prefix = item.name.toLowerCase().split("_").slice(0,3).join("_");
            const fuzzy  = filtered.find(f => f.name.toLowerCase().startsWith(prefix));
            if (fuzzy) { console.log("🔧 Fuzzy-Match:", fuzzy.name); return { name: fuzzy.name, cat: fuzzy.cat, img: imgPath(fuzzy) }; }
            return null;
          }
          // cat IMMER aus WARDROBE – nie aus Claudes Antwort
